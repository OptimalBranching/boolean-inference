...
impl BranchAndReduceProblem for RuleProblem {
    ...
    /// resulting domains as the returned sub-problem, and restore the scratch
    /// to base. Precondition (ob-core guarantee): called only single-level from
    /// the root, with CT scratch primed by `with_measure_scratch`.
    ///
