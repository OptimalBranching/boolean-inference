...
impl CaDiCaL<'_, '_> {
    ...
    /// Run CaDiCaL's scheduled learned-clause database reduction, if due.
    #[cfg(cadical_version = "v2.2.0")]
    pub fn maintain_learned_clauses(&mut self) {
        unsafe { ffi::ccadical_maintain_learned_clauses(self.handle) }
    }
